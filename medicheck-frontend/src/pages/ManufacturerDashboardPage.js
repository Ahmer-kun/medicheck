...
function ManufacturerDashboardPage({ batches, metamask, user, theme, onRefresh }) {
  ...
  // In ManufacturerDashboardPage.js - Update the fetchManufacturerCompanies function
    ...
    if (response.success) {
      ...
      const companiesWithStats = companiesData.map(company => {
        ...
        const manufacturerBatches = allBatches.filter(batch => {
          ...
          // Filter by manufacturer name AND ensure it's not a pharmacy medicine duplicate
          // Look for original manufacturer batches (not pharmacy-accepted duplicates)
        ...
        // Get accepted batches (these are the original batches with status 'accepted')
...
//       // Process each company to get batch counts
//       const companiesWithStats = await Promise.all(
//         companiesData.map(async (company) => {
//           try {
//             // Get batches for this manufacturer
//             const batchesResponse = await api.get(`/batches?manufacturer=${encodeURIComponent(company.companyName)}`);
//             const manufacturerBatches = batchesResponse.success ? batchesResponse.data : [];
...
//                 activeBatches
//               },
//               _batchCount: totalBatches // Keep for backward compatibility
//             };
//           } catch (error) {
//             console.error(`❌ Error getting stats for ${company.companyName}:`, error);
//             return {
//               ...company,
//               stats: {
//                 totalBatches: 0,
...
//       setManufacturerCompanies(companiesWithStats);
//       console.log("✅ Final processed companies:", companiesWithStats);
//     } else {
...
// };
  ...
  // SINGLE handleDeleteCompany function (remove any duplicates)
  ...
  // Replace the getCompanyStats function (around line 543) with this:
// const getCompanyStats = (companyId) => {
//   // First try to find company in the state and get its stats
//   const company = manufacturerCompanies.find(c => c._id === companyId);
  
//   if (company) {
//     // If company has its own batches array (from fetchManufacturerCompanies)
//     if (company.batches) {
//       const totalBatches = company.batches.length;
//       const verifiedBatches = company.batches.filter(b => b.blockchainVerified).length;
//       const expiredBatches = company.batches.filter(b => {
//         const expiryDate = new Date(b.expiry || b.expiryDate);
//         return expiryDate < new Date();
//       }).length;
//       const activeBatches = totalBatches - expiredBatches;
...
//     // Fallback to the old stats object
...
//   // If company not found, return defaults
//   return {
//     totalBatches: 0,
//     verifiedBatches: 0,
//     expiredBatches: 0,
...
const totalAcceptedBatches = manufacturerCompanies.reduce((sum, company) => {
  ...
  <div className="flex gap-1">
    <button
      onClick={() => {
        window.location.href = `/manufacturer?company=${company._id}`;
      }}
...
//       const response = await api.get("/manufacturer-companies");
//       if (response.success) {
//         setManufacturerCompanies(response.data);
//         console.log("✅ Loaded manufacturer companies with stats:", response.data.length);
        
//         // Pre-populate stats from the company data itself
//         const initialStats = {};
//         response.data.forEach(company => {
//           initialStats[company._id] = {
//             totalBatches: company.totalBatches || 0,
//             verifiedBatches: company.verifiedBatches || 0,
//             expiredBatches: company.expiredBatches || 0,
//             activeBatches: company.activeBatches || 0,
//             statusDistribution: []
//           };
//         });
...
//       console.log(`🔄 Fetching detailed stats for manufacturer: ${companyId}`);
      
//       const response = await api.get(`/manufacturer-companies/${companyId}/stats`);
//       if (response.success) {
//         setStats(prev => ({
//           ...prev,
//           [companyId]: response.data
//         }));
//         console.log(`✅ Detailed stats loaded for manufacturer ${companyId}`);
//       }
//     } catch (error) {
//       console.error(`❌ Error fetching stats for manufacturer ${companyId}:`, error);
//     } finally {
...
//             value={manufacturerCompanies.filter(c => c.isActive).length} 
//             gradient="bg-gradient-to-br from-green-50 to-green-100"
//             icon="✅"
//             compact={true}
//           />
...
//                               </div>
//                               <div className="text-gray-500 text-xs">
//                                 {companyStats.verifiedBatches} verified
//                               </div>
//                             </>
//                           )}
//                         </td>
...
//                   {selectedCompany.companyName} - Statistics
//                 </h3>
//                 <button
...
//                     {stats[selectedCompany._id].totalBatches}
//                   </div>
//                   <div className="text-xs text-gray-600">Total</div>
//                 </div>
//                 <div className="bg-white p-2 rounded border border-gray-200 text-center">
//                   <div className="text-lg font-bold text-green-600">
//                     {stats[selectedCompany._id].verifiedBatches}
//                   </div>
//                   <div className="text-xs text-gray-600">Verified</div>
//                 </div>
//                 <div className="bg-white p-2 rounded border border-gray-200 text-center">
//                   <div className="text-lg font-bold text-red-600">
//                     {stats[selectedCompany._id].expiredBatches}
//                   </div>
//                   <div className="text-xs text-gray-600">Expired</div>
//                 </div>
//                 <div className="bg-white p-2 rounded border border-gray-200 text-center">
//                   <div className="text-lg font-bold text-purple-600">
//                     {stats[selectedCompany._id].activeBatches || 
//                      (stats[selectedCompany._id].totalBatches - stats[selectedCompany._id].expiredBatches)}
//                   </div>

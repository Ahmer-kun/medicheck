...
// H3 FIX: Accept batches as a prop from App.js (already loaded there)
// so we don't double-fetch on every /analytics visit.
function AnalyticsPage({ metamask, user, theme, batches: propBatches = [] }) {
  ...
  // H3 FIX: Use prop batches if available, else fetch independently
  useEffect(() => {
    if (propBatches && propBatches.length > 0) {
      console.log(`📊 Using ${propBatches.length} batches from App.js prop — no extra fetch needed`);
      computeAnalytics(propBatches);
    } else {
  ...
  // H2 FIX: Use expiryDate only (no more batch.expiry || batch.expiryDate)
  const calculateMedicineStats = (batches) => {
    ...
    const expiredCount = batches.filter(batch => {
      const expiryDate = normalizeDate(new Date(batch.expiryDate));
  ...
  const calculateMonthlyTrend = (batches) => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      months.push({ month: monthName, registered, expired });
    }
  ...
  // H3 FIX: Extracted compute function — used by both the prop path and fetch path
  const computeAnalytics = (allBatches) => {
  ...
  // Fallback: only called when propBatches is empty (e.g. hard refresh to /analytics)
  const fetchRealAnalyticsData = async () => {
    try {
      setLoading(true);
      console.log("🔄 No prop batches — fetching from API...");

      const batchesResponse = await api.get("/batches");

      if (batchesResponse.success) {
        const allBatches = batchesResponse.data || [];
        console.log(`📊 Fetched ${allBatches.length} batches from API`);
        computeAnalytics(allBatches);
      } else {
        console.error("❌ Failed to fetch batches");
        setAnalyticsData({
          overview: { totalMedicines: 0, activeMedicines: 0, expiredMedicines: 0, nearExpiry: 0, totalBatches: 0, verifiedBatches: 0 },
          statusDistribution: [],
          monthlyTrend: [],
          manufacturerStats: []
        });
        ...
        <ResponsiveContainer>
          <div className="py-4 md:py-6 space-y-4 md:space-y-6 min-h-screen">
            ...
            <div className="mb-3 md:mb-6">
              <h1 className="heading-1 mb-1 md:mb-2">Analytics Dashboard</h1>
              <p className="text-body">Comprehensive analytics and insights from blockchain network</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 lg:gap-6 mb-4 md:mb-6">
              <Card title="Total Medicines" value={overview?.totalMedicines || 0} gradient="bg-gradient-to-br from-blue-50 to-blue-100" icon="💊" compact={true} />
              <Card title="Active Medicines" value={overview?.activeMedicines || 0} gradient="bg-gradient-to-br from-green-50 to-green-100" icon="✅" compact={true} />
              <Card title="Expired Medicines" value={overview?.expiredMedicines || 0} gradient="bg-gradient-to-br from-red-50 to-red-100" icon="❌" compact={true} />
            </div>
                      ...
                      <div key={index} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500">
                            {entry.value} ({overview?.totalMedicines > 0 ? ((entry.value / overview.totalMedicines) * 100).toFixed(1) : 0}%)
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="h-64 md:h-[300px]">
                  <ChartContainer width="100%" height="100%">
                    <LineChart data={monthlyTrend}>
                      ...
                      <XAxis dataKey="month" stroke="#6b7280" fontSize={isMobile ? 10 : 12} tickMargin={5} />
                      ...
                      <Tooltip
                        ...
                        formatter={(value, name) => [`${value} batches`, name === 'registered' ? 'Registered' : 'Expired']}
                      />
                      <Legend
                        wrapperStyle={{ fontSize: isMobile ? '10px' : '12px', paddingTop: isMobile ? '5px' : '10px' }}
                        formatter={(value) => value === 'registered' ? 'Registered' : 'Expired'}
                      />
                      <Line type="monotone" dataKey="registered" stroke="#3B82F6" strokeWidth={isMobile ? 2 : 3} dot={{ r: isMobile ? 3 : 4, fill: '#3B82F6' }} activeDot={{ r: isMobile ? 4 : 6 }} name="registered" />
                      <Line type="monotone" dataKey="expired" stroke="#EF4444" strokeWidth={isMobile ? 2 : 3} dot={{ r: isMobile ? 3 : 4, fill: '#EF4444' }} activeDot={{ r: isMobile ? 4 : 6 }} name="expired" />
                    </LineChart>
                  </ChartContainer>
                </div>
            ...
            <section className="responsive-card mb-4 md:mb-6">
              ...
              <div className="h-64 md:h-[300px]">
                <ChartContainer width="100%" height="100%">
                  ...
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                    <XAxis dataKey="name" stroke="#6b7280" angle={-45} textAnchor="end" height={isMobile ? 50 : 80} fontSize={isMobile ? 9 : 11} interval={0} />
                    <YAxis stroke="#6b7280" fontSize={isMobile ? 10 : 12} width={isMobile ? 30 : 40} />
                    <Tooltip
                      contentStyle={{
                        backgroundColor: 'rgba(255, 255, 255, 0.95)',
                      ...
                      formatter={(value) => [`${value} batches`, 'Total Batches']}
                    />
                    <Bar dataKey="batches" fill="#3B82F6" radius={[4, 4, 0, 0]} name="Batches" />
                  </BarChart>
                </ChartContainer>
              </div>
            </section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-lg md:text-xl font-bold text-green-600">
                      {overview?.totalBatches > 0 ? `${Math.round((overview.verifiedBatches / overview.totalBatches) * 100)}%` : '0%'}
                    </div>
                  </div>
                  <div className="text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-2 md:gap-3 mt-4 md:mt-6">
              ...
              <button
                onClick={() => {
                  const exportData = { overview, statusDistribution, monthlyTrend, manufacturerStats, exportedAt: new Date().toISOString() };

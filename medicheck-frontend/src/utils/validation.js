...
export const validateBatch = (batch) => {
  ...
  // FIX: was checking batch.expiry — now checks batch.expiryDate
  // ManufacturerPage form submits expiryDate; this was causing silent
  // validation failure (❌ Validation failed on ManufacturerPage.js:305)
  const expiryValue = batch.expiryDate || batch.expiry; // fallback for safety
  if (!expiryValue) {

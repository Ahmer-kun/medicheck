...
/* --------------------------------------------
   Verify Pharmacy Medicine (DB Check)
-------------------------------------------- */
...
/* --------------------------------------------
   ACCEPT & VERIFY MANUFACTURER BATCH 
-------------------------------------------- */
export const acceptManufacturerBatch = async (req, res) => {
  try {
    ...
    const pharmacyMedicine = new PharmacyMedicine({
      ...
      pharmacyName: pharmacyCompany.name, // Uses company name, & not user.pharmacyName
      status: 'In Stock',
    ...
    try {
      ...
      // Here we use correct method name - this might need adjustment based on our blockchain service

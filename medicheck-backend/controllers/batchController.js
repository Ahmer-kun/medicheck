...
export const createBatch = async (req, res) => {
  try {
    const {
      ...
      manufactureDate,
      expiryDate,
      expiry,           // legacy fallback — frontend used to send `expiry`
      formulation,
      manufacturer,
      pharmacy,
      quantity,
    ...
    // Accept either expiryDate (new standard) or expiry (legacy)
    const rawExpiry = expiryDate || expiry;

    console.log("Creating batch with STRICT DUAL STORAGE:", { batchNo });

    // Validate required fields
    if (!batchNo || !name || !manufactureDate || !rawExpiry || !quantity || !manufacturer) {
      return res.status(400).json({
        success: false,
        message: "Missing required fields: batchNo, name, manufactureDate, expiryDate, quantity, manufacturer"
      });
    }
    ...
    const validExpiryDate = validateDate(rawExpiry, 'expiryDate');
      ...
      try {
        ...
        try {
          ...
          const blockchainData = {
            ...
            expiryDate: formattedBatch.expiryDate,

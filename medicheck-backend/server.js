...
app.get('/api/test-contract-methods', async (req, res) => {
  try {
    const BlockchainService = (await import('./services/blockchainService.js')).default;
    // await BlockchainService.initialize();
    
    // Test 1: Get all batch numbers (should be empty array initially)
    const allBatches = await BlockchainService.contract.methods.getAllBatchNumbers().call();
    
    // Test 2: Get owner
    const owner = await BlockchainService.contract.methods.owner().call();
    
    // Test 3: Check if a test batch exists (should be false)
    const testExists = await BlockchainService.contract.methods.verifyMedicineExistence('TEST123').call();
...
app.get('/api/test-contract-abi-match', async (req, res) => {
  try {
    const web3 = new Web3(process.env.BLOCKCHAIN_NETWORK);
    const contractABI = require('./contracts/MedicineTrackerABI.json');
    const contract = new web3.eth.Contract(contractABI, process.env.CONTRACT_ADDRESS);
    
    // Test 1: Check method signatures
    const methods = Object.keys(contract.methods);
    
    // Test 2: Try to get contract bytecode hash
    const code = await web3.eth.getCode(process.env.CONTRACT_ADDRESS);
    const codeHash = web3.utils.sha3(code);
    
    res.json({
      success: true,
      contractAddress: process.env.CONTRACT_ADDRESS,
      totalMethods: methods.length,
      methodsAvailable: methods.includes('registerMedicine') && methods.includes('getMedicine'),
      hasBytecode: code.length > 2,
      bytecodeHash: codeHash,
      expectedMethods: ['registerMedicine', 'getMedicine', 'verifyMedicineExistence', 'getAllBatchNumbers']
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
app.post('/api/test-register-batch', async (req, res) => {
  try {
    const BlockchainService = (await import('./services/blockchainService.js')).default;
    
    const testBatch = {
      batchNo: 'TEST' + Date.now().toString().slice(-6),
      name: 'Test Medicine',
      ...
      packaging: { packSize: '1X10' },
      status: 'active'
    };
    
    console.log('🧪 Testing batch registration with data:', testBatch);
    
    const result = await BlockchainService.registerCompleteMedicine(testBatch);
    
    res.json({
      success: true,
      message: 'Test batch registered successfully',
...
// Debug private key endpoint (REMOVE IN PRODUCTION)
app.get('/api/debug/private-key-check', async (req, res) => {
  try {
    ...
    if (!privateKey) {
      return res.json({
        success: false,
        message: 'No private key in environment'
      });
    }
    ...
    if (cleanKey.length !== 66) {
      issues.push(`Length should be 66 characters, got ${cleanKey.length}`);
    }
    
    if (!cleanKey.startsWith('0x')) {
      issues.push('Should start with 0x');
    }
    ...
    try {
      ...
      issues.push(`Failed to derive address: ${deriveError.message}`);
    }
    ...
    res.json({
      ...
      privateKeyInfo: {
        ...
        startsWith0x: cleanKey.startsWith('0x'),
        hexPartLength: hexPart.length,
        hexPartValid: /^[0-9a-fA-F]+$/.test(hexPart),
        issues: issues.length > 0 ? issues : 'None',
        derivedAddress: derivedAddress,
        matchesExpected: derivedAddress === process.env.DEPLOYER_ADDRESS
      }
    });
...
//      REPAIR ALL BATCHES ENDPOINT

app.post('/api/repair-all-batches', async (req, res) => {
  try {
    const Batch = (await import('./models/Batch.js')).default;
    const PharmacyMedicine = (await import('./models/PharmacyMedicine.js')).default;
    const BlockchainService = (await import('./services/blockchainService.js')).default;
    ...
    console.log('🔧 Repairing ALL batches...');
    
    // Get all batches from MongoDB that should be on blockchain
    const batches = await Batch.find({
      $or: [
        { blockchainVerified: true },
        { blockchainTransactionHash: { $exists: true } }
      ]
    });
    
    console.log(`Found ${batches.length} batches to repair`);
    ...
    for (const batch of batches) {
      try {
        console.log(`🔧 Repairing batch: ${batch.batchNo}`);
        
        // Check if already on new blockchain
        const exists = await BlockchainService.contract.methods.verifyMedicineExistence(batch.batchNo).call();
        
        if (exists) {
          console.log(`✅ Batch ${batch.batchNo} already on blockchain`);
          report.details.push({
            batchNo: batch.batchNo,
        ...
        const blockchainData = {
          batchNo: batch.batchNo,
          name: batch.name,
          medicineName: batch.medicineName || batch.name,
          manufactureDate: batch.manufactureDate,
          expiryDate: batch.expiry,
          formulation: batch.formulation,
          quantity: batch.quantity,
          manufacturer: batch.manufacturer,
          pharmacy: batch.pharmacy || "To be assigned",
          packaging: batch.packaging || {},
          status: batch.status || "active"
        };
        
        // Register on new blockchain
        const result = await BlockchainService.registerCompleteMedicine(blockchainData);
        
        // Update MongoDB
        batch.blockchainVerified = true;
        batch.blockchainTransactionHash = result.transactionHash;
        batch.blockchainBlockNumber = result.blockNumber;
        batch.blockchainError = null;
        await batch.save();
        
        console.log(`✅ Batch ${batch.batchNo} repaired: ${result.transactionHash}`);
        ...
        console.error(`❌ Failed to repair ${batch.batchNo}:`, error.message);
        
        batch.blockchainVerified = false;
        batch.blockchainError = `Repair failed: ${error.message}`;
        await batch.save();
    ...
    res.json({
      success: true,
      message: `Repaired ${report.repaired}/${report.total} batches`,
      report: report
    });
...
app.post('/api/repair-pharmacy-medicines', async (req, res) => {
  try {
    const PharmacyMedicine = (await import('./models/PharmacyMedicine.js')).default;
    const BlockchainService = (await import('./services/blockchainService.js')).default;
    ...
    console.log(`Found ${medicines.length} pharmacy medicines to repair`);
    ...
    for (const medicine of medicines) {
      try {
        console.log(`🔧 Repairing pharmacy medicine: ${medicine.batchNo}`);
        
        const exists = await BlockchainService.contract.methods.verifyMedicineExistence(medicine.batchNo).call();
        
        if (exists) {
          console.log(`✅ Already on blockchain`);
          report.repaired++;
          continue;
        }
        ...
        const blockchainData = {
          ...
          quantity: medicine.quantity,
          manufacturer: medicine.manufacturer,
          pharmacy: medicine.pharmacyName || "Pharmacy",
          packaging: medicine.packaging || {},
        ...
        console.log(`✅ Repaired: ${medicine.batchNo}`);
        
        report.repaired++;
        
      } catch (error) {
        console.error(`❌ Failed: ${medicine.batchNo}`, error.message);
        
        medicine.blockchainVerified = false;
        medicine.blockchainError = error.message;
        await medicine.save();
    ...
    res.json({
      success: true,
      message: `Repaired ${report.repaired}/${report.total} pharmacy medicines`,
      report: report
    });
...
// In server.js - replace the verify-contract-deployment endpoint
...
// API TEST CONTRACT CALL
// Add to server.js - test contract interaction
app.get('/api/test-contract-call', async (req, res) => {
  try {
    const Web3 = await import('web3');
    const { readFile } = await import('fs/promises');
    const { join, dirname } = await import('path');
    ...
    const allBatches = await contract.methods.getAllBatchNumbers().call();
    
    // Test 3: Check if a test batch exists
    const testExists = await contract.methods.verifyMedicineExistence('TEST123').call();
    
    // Test 4: Get contract bytecode
    const code = await web3.eth.getCode(contractAddress);
    ...
    res.json({
      ...
      owner: owner,
      totalBatches: allBatches.length,
      batches: allBatches.slice(0, 5), // First 5 only
      testBatchExists: testExists,
      hasCode: code !== '0x',
      methods: Object.keys(contract.methods).filter(key => 
        typeof contract.methods[key] === 'function'
      ),
      network: await web3.eth.net.getId()
    });
...
app.get('/api/debug-private-key', async (req, res) => {
  try {
    ...
    if (!privateKey) {
      return res.json({ 
        success: false, 
        message: 'DEPLOYER_PRIVATE_KEY not set in .env' 
      });
    }
    ...
    const analysis = {
      ...
      trimmedLength: privateKey.trim().length,
      startsWith0x: privateKey.startsWith('0x'),
      endsWithNewline: privateKey.endsWith('\n'),
      containsSpaces: privateKey.includes(' '),
      containsNewlines: privateKey.includes('\n'),
      first20Chars: privateKey.substring(0, 20),
      last20Chars: privateKey.substring(privateKey.length - 20),
      hexPart: privateKey.substring(2),
      hexPartLength: privateKey.substring(2).length,
      isValidLength: privateKey.length === 66,
      charAnalysis: privateKey.split('').map((char, i) => ({
        position: i,
        char: char,
        charCode: char.charCodeAt(0),
        isHex: /[0-9a-fA-Fx]/.test(char)
      }))
    };
    ...
    let accountInfo = { error: null };
    try {
      const account = web3.eth.accounts.privateKeyToAccount(privateKey);
      accountInfo = {
        address: account.address,
        success: true,
        addressMatchesExpected: account.address.toLowerCase() === process.env.DEPLOYER_ADDRESS?.toLowerCase()
      };
    } catch (error) {
      accountInfo = {
        error: error.message,
    ...
    res.json({
      success: true,
      analysis: analysis,
      accountCreation: accountInfo,
      recommendations: [
        !analysis.startsWith0x ? 'Add 0x prefix' : null,
        analysis.length !== 66 ? `Should be 66 chars, got ${analysis.length}` : null,
        analysis.containsSpaces ? 'Remove all spaces' : null,
        analysis.containsNewlines ? 'Remove newlines' : null
      ].filter(Boolean)
    });
...
app.get('/api/test-sign-transaction', async (req, res) => {
  try {
    const Web3 = await import('web3');
    ...
    // Test 1: Check address matches
    const web3Account = web3.eth.accounts.privateKeyToAccount(privateKey);
    const ethersAddress = wallet.address;
    
    console.log('Address comparison:', {
      web3: web3Account.address,
      ethers: ethersAddress,
      matches: web3Account.address.toLowerCase() === ethersAddress.toLowerCase()
    });
    ...
    const testTx = {
      ...
      nonce: await web3.eth.getTransactionCount(web3Account.address),
      chainId: 11155111,
      data: '0x'
    };
    ...
    let web3Signed = null;
    try {
      web3Signed = await web3.eth.accounts.signTransaction(testTx, privateKey);
      console.log('Web3 signing:', web3Signed ? 'SUCCESS' : 'FAILED');
    } catch (web3Error) {
      console.log('Web3 signing error:', web3Error.message);
    }
    
    // Sign with ethers
    let ethersSigned = null;
    try {
      const ethersTx = {
        to: testTx.to,
        value: 0n,
        gasLimit: BigInt(testTx.gas),
        gasPrice: BigInt(testTx.gasPrice),
        nonce: testTx.nonce,
        chainId: testTx.chainId,
        data: testTx.data
      };
      ethersSigned = await wallet.signTransaction(ethersTx);
      console.log('Ethers signing:', ethersSigned ? 'SUCCESS' : 'FAILED');
    } catch (ethersError) {
      console.log('Ethers signing error:', ethersError.message);
    }
    
    // Test 3: Try to send 0-value transaction
    let txHash = null;
    if (ethersSigned) {
      try {
        const receipt = await web3.eth.sendSignedTransaction(ethersSigned);
    ...
    res.json({
      success: true,
      addressesMatch: web3Account.address.toLowerCase() === ethersAddress.toLowerCase(),
      web3Signing: web3Signed ? 'success' : 'failed',
      ethersSigning: ethersSigned ? 'success' : 'failed',
      transactionSent: txHash || 'failed',
      balance: await web3.eth.getBalance(web3Account.address),
      network: await web3.eth.net.getId()
    });
...
app.get('/api/test-email', async (req, res) => {
  try {
    const testUser = {
      ...
      name: 'Test User', 
      role: 'admin',
      email: 'muhammadahmer1qw2@gmail.com' // Your test email
    };
...
// Add this import at the top of server.js (after other imports)
// ============================================
// FIXED: /api/system/health/detailed ROUTE
// ============================================
// Add this code RIGHT BEFORE the 404 handler
// Look for this line: app.use('*', (req, res) => { ... }
// And add the routes BEFORE it
...
// async function safeModelCount(modelName, filter = {}) {
//   try {
...
// Replaced updated the /api/blockchain/real/status route in server.js
// app.get('/api/blockchain/real/status', async (req, res) => {
//   try {
//     console.log('🔍 Checking real blockchain status...');
...
//     try {
//       // Try to import the real blockchain service
//       const realBlockchainService = await import('./services/realBlockchainService.js');
      
//       if (realBlockchainService && realBlockchainService.default) {
//         const service = realBlockchainService.default;
        
//         // Check if service has getNetworkInfo method
//         if (typeof service.getNetworkInfo === 'function') {
//           blockchainInfo.details = await service.getNetworkInfo();
//           blockchainInfo.available = true;
//           blockchainInfo.error = null;
//         } else {
//           blockchainInfo.error = 'getNetworkInfo method not available';
//         }
//       } else {
//         blockchainInfo.error = 'RealBlockchainService not found';
//       }
      
//     } catch (importError) {
//       console.error('Real blockchain service import error:', importError.message);
//       blockchainInfo.error = `Service import failed: ${importError.message}`;
//       blockchainInfo.details = {
//         fileExists: await checkFileExists('./services/realBlockchainService.js'),
//         stack: process.env.NODE_ENV === 'development' ? importError.stack : undefined
//       };
//     }
    
//     // Also check basic blockchain connection as fallback
//     try {
//       const Web3 = await import('web3');
//       const web3 = new Web3.default(process.env.BLOCKCHAIN_NETWORK || 'http://localhost:8545');
//       const blockNumber = await web3.eth.getBlockNumber();
//       blockchainInfo.fallback = {
//         connected: true,
